...
def setup_train_args():
    ...
    parser.add_argument('--dataset_path', default='data/FB15K', type=str, required=False,help='数据集根目录')
    parser.add_argument('--entity_path', default='data/FB15K/entity2id.txt', type=str, required=False, help='实体id文件')
    parser.add_argument('--concept_path', default='data/FB15K/entity2id.txt', type=str, required=False, help='概念id文件')
    parser.add_argument('--relation_path', default='data/FB15K/relation2id.txt', type=str, required=False, help='关系id文件')
    ...
    parser.add_argument('--triple_path', default='train2id.txt', type=str, required=False,help='triple id文件')

    parser.add_argument('--instanceOf_path_neg_test', default='data/FB15K/test2id.txt', type=str, required=False,help='测试集instanceOf负样本')
    ...
    parser.add_argument('--triple_path_neg_test', default='data/FB15K/test2id_negative.txt', type=str,
                        required=False, help='测试集triple负样本')
    parser.add_argument('--triple_path_pos_test', default='data/FB15K/test2id_positive.txt', type=str,
    ...
    parser.add_argument('--B_t', default=2, type=float, required=False, help='超参数B_t')
    parser.add_argument('--B_r', default=2, type=float, required=False, help='超参数B_r')
    parser.add_argument('--sp', default=" ", type=str, required=False, help='文件分隔符')
...
def main():
    ...
    args.triple_path = args.dataset_path + "/train2id.txt"
    args.instanceOf_path = args.triple_path
    args.subClassOf_path = args.triple_path
    ...
    logger.info("")
    dataset_triple = TripleDataset(args.entity_path,args.concept_path,args.relation_path,args.instanceOf_path,args.triple_path,triple=True,rate=args.rate,sp="\t")
    ...
    model_path = "model/model_2021-05-24-11/model_epoch70.pt"
    loadModel(model_path, model, optimizer)

    model.eval()
    batch_train = torch.from_numpy(np.array(dataset_triple.posTriples))
    linkPredictor.evaluate_triple_raw(model, triple_pos)
    linkPredictor.evaluate_triple_filter(model,triple_pos,batch_train)
